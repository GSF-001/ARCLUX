...
- [x] Atmospheric scattering `SPACE -> SURFACE` + volumetric clouds (`coverage/density/altitude/thickness/wind`) + cloud intersection (`CLEAR -> EDGE -> INTERIOR`) - DONE 10.X.1 (part of godRays/fog)
- [x] Weather stack `CLEAR/OVERCAST/RAIN/STORM` coordinated - DONE 10.X.2 weatherStack.ts
- [x] Rain + rain–surface/ocean (`wet/puddles/runoff/reflection`, `micro-ripples`) - DONE 10.X.2 rain.ts
- [x] Lightning event chain (`LightningEvent` -> cloud flash -> terrain/ocean/facility + reflection) - DONE 10.X.2 lightning.ts
- [x] Fog (+ fog–sun god-ray feed) + vegetation wind/rain (`grass->tree` phased) - DONE 10.X.3 fog.ts + vegetation.ts
